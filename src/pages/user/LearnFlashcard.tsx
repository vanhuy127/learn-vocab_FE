  ...
  if (!currentCard || (isTrackingProgress && studySet && studySet.items.length === 0)) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 text-center">
        <h2 className="text-xl font-semibold">🎉 Không có từ nào cần ôn ngay bây giờ</h2>

        <p className="text-muted-foreground">Tất cả từ vựng đã được ôn đúng lịch. Hãy quay lại sau nhé!</p>

        <button className="bg-primary rounded-md px-4 py-2 text-white hover:opacity-90" onClick={() => navigate(-1)}>
          Quay lại
        </button>
      </div>
    );
