...
const StudySetDetails = () => {
  ...
  const { getAdminStudySetById, getAdminStatisticsStudySetById } = useStudySetService();
        ...
        <div className="h-full rounded-xl border border-gray-100 p-6 shadow-sm">
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="space-y-2">
              ...
              <p className="bg-muted rounded p-3 text-sm">{studySet.isDeleted ? 'Đã xóa' : 'Chưa xóa'}</p>
            </div>

            <Button variant={'destructive'} disabled={!studySet.isDeleted} className="w-full">
              Khôi phục
            </Button>
          </div>
        </div>
